...
#[rstest(mock_rpc_data("get_live_cell", "unknown"))]
fn get_live_cell_unknown(mock_rpc_data: MockRpcData) {
    let ckb_client = mock_rpc_data.client();
    ...
    // 调用被测试的函数
    let response = ckb_client.get_live_cell_with_include_tx_pool(out_point, with_data, true).unwrap();
    // 反序列化和比较输出结果
    assert_eq!(response.status, mock_rpc_data.response_data["result"]["status"].clone().as_str().unwrap().to_string());
    assert_eq!(response.cell.is_none(),true);
}
