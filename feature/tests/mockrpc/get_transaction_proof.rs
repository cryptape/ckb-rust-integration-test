...
fn get_transaction_proof_with_tx_hashes_and_block_hash(mock_rpc_data: MockRpcData) {
...
fn get_transaction_proof_with_tx_hashes_and_null(mock_rpc_data: MockRpcData) {
    let ckb_client = mock_rpc_data.client();
...
fn get_transaction_proof_with_tx_hashes(mock_rpc_data: MockRpcData) {
    ...
    assert!(false,"get_transaction_proof/[tx_hashes]")
    // 反序列化输入参数
    // let tx_hashes = serde_json::from_value(mock_rpc_data.request_data["params"][0].clone()).unwrap();
