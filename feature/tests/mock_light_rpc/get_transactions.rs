...
#[rstest(mock_rpc_data("get_transactions", "groupedTxWithCells:rust"))]
fn get_transactions(mock_rpc_data: MockRpcData) {
    println!("method:{}",mock_rpc_data.method);
    println!("params:{}",mock_rpc_data.params);
    let ckb_light_client = mock_rpc_data.client();

    let ret= ckb_light_client.get_transactions(
...
#[rstest(mock_rpc_data("get_transactions", "ungroupedTxWithCell:rust"))]
fn get_transactions_ungrouped_tx_with_cell(mock_rpc_data: MockRpcData) {
    println!("method:{}",mock_rpc_data.method);
    println!("params:{}",mock_rpc_data.params);
    let ckb_light_client = mock_rpc_data.client();
